...
{
    public partial class Product_A : UserControl
    {
        public readonly string Product_Style_A_Html = Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory + "Html\\Template\\ProductStyle\\product_sylte_a.html");
        ...
        public string Get_Pending_Join_Html_Content()
        {
            ...
            try
            {
                using (StreamReader sr = new StreamReader(Product_Style_A_Html))
                {
                    strHtml = sr.ReadToEnd();
                }
                strHtml = string.Format(strHtml, textBox_URL.Text.Trim() + MainFrm.strLinkPart, imgPath, textBox_ProductName.Text,
                    textBox_Reference.Text, MainFrm.CurrentCurrency + textBox_Trick.Text, MainFrm.CurrentCurrency + textBox_Price.Text, textBox_ViewMore.Text + MainFrm.strLinkPart, MainFrm.ViewMoreImgPath);
